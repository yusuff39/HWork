...
# area
    ...
    def assistant_add_screen(self,y_=1,t_y_=10,root_title ="add",heihgt___=None):
        ...
        for key_ in self.json_X[0].keys():  
            if key_=='id':
                val_new = self.worker_add_screen(seq_row=y_,total_row=t_y_,seq_col=n,total_col=buff_2,next_col=(self.seq_x[i+1]),height_=add_s_mh,val__=buf_3,State_='readonly') 
                add_value[key_] = val_new
                print(val_new)
            else:
                val_n = self.worker_add_screen(seq_row=y_,total_row=t_y_,seq_col=n,total_col=buff_2,next_col=(self.seq_x[i+1]),height_=add_s_mh,val__=key_) 
                add_value[key_] = val_n 
            n += self.seq_x[i+1]
            i+=1
            left_indent += 1
    ...
    def worker_add_screen(self,seq_row,total_row,seq_col,total_col,next_col,col_=None,height_=None,val__='',State_='normal'):
        ...
        key_gen = None
        font_x = 15 if self.MW2 >= 2000 else 10        
        ent_ = tk.Entry(self.root_add, font=("Helvetica", font_x),justify='center')
        BUFF1 = int((seq_row/total_row)*height_)
        BUFF2 = int((seq_col/total_col)*(self.MW2-200))
        ...
        ent_.place(x=BUFF2,y=BUFF1,width=buff,height=50)
        ...
        ent_.insert(0, val__)
        ent_.configure(state=State_)
        
        ent_.bind("<FocusOut>", entry_click)
        ent_.bind("FocusIn",change_val)
